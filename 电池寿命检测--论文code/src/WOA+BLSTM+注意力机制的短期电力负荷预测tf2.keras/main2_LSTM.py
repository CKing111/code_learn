...
from model import LSTM_,result,split_data
...
if train_again:
    ...
    plt.legend()
    plt.savefig('figure/lstm_model_loss_curve.jpg')
else:  # 加载模型
...
result(test_label1,test_pred1,'LSTM')
...
plt.xlabel('样本点')
plt.ylabel('功率')
plt.savefig('figure/LSTM预测结果.jpg')
plt.show()
