...
import pandas as pd
from model import BiLSTM,  result,split_data
...
model = BiLSTM(sequence, feature, hidden1,hidden2, fc, output_node).build_model()
...
if train_again:
    ...
    plt.legend()
    plt.savefig('figure/bilstm_loss_curve.jpg')
else:  # 加载模型
...
np.savez('result/bilstm_result.npz', true=test_label1, pred=test_pred1)
# In[]计算各种指标
test_pred1=test_pred1.reshape(-1,1)
test_label1=test_label1.reshape(-1,1)

result(test_label1,test_pred1,'BiLSTM')
...
plt.xlabel('样本点')
plt.ylabel('功率')
plt.savefig('figure/BiLSTM预测结果.jpg')
plt.show()
