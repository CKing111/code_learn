...
def imputation_with_spline(file, to_file):
    ...
    def date_trend(group):
        ...
        if group.link_ID.values[0] == '3377906280028510514':
            fig = plt.figure(figsize = (16,8))
            plt.plot(tmp.index, tmp['date_trend'], 'o', tmp.index, tmp['travel_time'], 'ro')
            plt.xticks(fontsize=14)
            plt.yticks(fontsize=14)

            plt.title(group.link_ID.values[0],fontsize=14)
            plt.show()
            fig.savefig('date_trend.svg',format='svg')
    ...
    def minute_trend(group):
        ...
        if group.link_ID.values[0] == '3377906280028510514':
            ...
            plt.title(group.link_ID.values[0],fontsize=14)
            plt.show()
            fig.savefig('minute_trend.svg',format='svg')
...
def create_feature(file, to_file, lagging=5):
    ...
    df2.boxplot(by=['links_num'], column='travel_time',figsize=(16,8),fontsize=16)
    plt.xticks(fontsize=14)
    plt.yticks(fontsize=14)
    plt.show()
    ...
    df2.boxplot(by=['week_hour'], column='travel_time',figsize=(16,8),fontsize=16)
    plt.xticks(fontsize=14)
    plt.yticks(fontsize=14)
    plt.show()
