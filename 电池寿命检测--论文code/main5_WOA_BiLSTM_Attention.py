...
# In[] 采用WOA优化BiLSTM-Attention的 lr num_epochs batch_size hidden1 hidden2 fc 6个参数
optim_again = True  # 为False的时候就直接加载之间优化好的参建模
# 训练模型
if optim_again:
    best,trace,process=WOA(train_data,train_label,test_data,test_label)
    trace,process=np.array(trace),np.array(process)
    np.savez('result/woa_result.npz',trace=trace,best=best,process=process)
else:
    data=np.load('result/woa_result.npz')
...
hidden1 = int(best[3])  # bilstm节点数1
hidden2 = int(best[4])  # bilstm节点数2
fc = int(best[5]) #全连接层节点数
...
if train_again:
    ...
    model.save('model/woa_bilstm_attention_model.h5')
    # 画loss曲线
    plt.figure()
    plt.ylabel('MSE')
    plt.xlabel('Epoch')
    ...
    plt.legend()
    plt.savefig('figure/woa_bilstm_attention_loss_curve.jpg')
else:  # 加载模型
    model = tf.keras.models.load_model('model/woa_bilstm_attention_model.h5')
...
np.savez('result/woa_bilstm_attention_result.npz', true=test_label1, pred=test_pred1)
# In[]计算各种指标
test_pred1=test_pred1.reshape(-1,1)
test_label1=test_label1.reshape(-1,1)

result(test_label1,test_pred1,'WOA_BiLSTM_attention')
...
plt.xlabel('样本点')
plt.ylabel('功率')
plt.savefig('figure/WOA_BiLSTM_attention预测结果.jpg')
plt.show()
