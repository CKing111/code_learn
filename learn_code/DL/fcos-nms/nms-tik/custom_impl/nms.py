...
class OneCoreNMS():
    def __init__(self, tik_instance, nms_param):
        self.total_input_proposal_num = nms_param[0]
        self.total_output_proposal_num = nms_param[1]
        ...
        self.selected_proposals.set_as(0)
    ...
    def ask_for_ub(self):
        out_num_align_up = ceil_div(self.total_output_proposal_num, 16) * 16
    ...
    def nms_single_core_subprocess_three(self):
        with self.tik_instance.for_range(
                                        0, self.handling_proposals) as i:
            with self.tik_instance.if_scope(
                    tik.all(self.temp_supvec_ub[i] == 0,
                        self.selected_proposals <
                        self.total_output_proposal_num,
                        i + self.handled_proposal_cnt <
    ...
    def nms_single_core_subprocess_two(self, length, threshold):
        with self.tik_instance.for_range(0, ceil_div(
                            self.handling_proposals, 16)) as i:
