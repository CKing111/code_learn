...
class FCOSPostProcessor(torch.nn.Module):
    ...
    def forward_for_single_feature_map(                     #对FPN金字塔的每一层（P3，P4,P5,P6,P7）的输出结果进行后处理
        ...
        self.tik_instance.data_move(box_regression,box_regression.reshape(N,4,H,W), 0, 1, 32, 0, 0)
        self.tik_instance.data_move(box_regression,box_regression.transpose(0,2,3,1), 0, 1, 32, 0, 0)
        # 预测回归边框 重新调整其shape
        self.tik_instance.data_move(box_regression,box_regression.reshape(N, -1, 4), 0, 1, 32, 0, 0)

        #centerness = centerness.view(N, 1, H, W).permute(0, 2, 3, 1) # 将其展成一维，并将其维度进行换位
        self.tik_instance.data_move(centerness,centerness.reshape(N,1,H,W), 0, 1, 32, 0, 0)
        self.tik_instance.data_move(centerness,centerness.transpose(0,2,3,1), 0, 1, 32, 0, 0)
        #预测的中心度数据
        self.tik_instance.data_move(centerness,centerness.reshape(N, -1).sigmoid(), 0, 1, 32, 0, 0)
        ...
        i = self.tik_instance.Scalar(dtype="int32", name="i")
       ...
       with self.tik_instance.for_range(0,N) as i:
            ...
            # ], dim=1)
            self.tik_instance.data_move(detections, np.stack([
                # x1 = x -l
                per_locations[:, 0] - per_box_regression[:, 0],
                # y1= y-t
                per_locations[:, 1] - per_box_regression[:, 1],
                ...
                # y2 = y+b
                per_locations[:, 1] + per_box_regression[:, 3],
            ],axis = 0), 0, 1, 32, 0, 0)               #axis = 0表示增加第一维度         
    ...
    def compute_locations(self, features):
            tik_instance = tik.Tik()
                ...
                self.locations_per_level = self.tik_instance.Tensor(self.dtype,locations_shape, 
                        name="locations_per_level", scope=tik.scope_gm)                    
