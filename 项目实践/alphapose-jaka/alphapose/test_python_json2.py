...
# cmd = ['python','/home/cxking/AlphaPose/scripts/demo_inference_test2.py',' --cfg ','/home/cxking/AlphaPose/configs/coco/resnet/256x192_res50_lr1e-3_2x-dcn.yaml ','--checkpoint ','/home/cxking/AlphaPose/pretrained_models/fast_dcn_res50_256x192.pth ','--outdir',' /home/cxking/AlphaPose/examples/res ','--vis ','--webcam',' 0']
alphapose_cmd = ['python /home/cxking/AlphaPose/scripts/demo_inference_test2.py --cfg /home/cxking/AlphaPose/configs/coco/resnet/256x192_res50_lr1e-3_2x-dcn.yaml --checkpoint /home/cxking/AlphaPose/pretrained_models/fast_dcn_res50_256x192.pth --outdir /home/cxking/AlphaPose/examples/res --vis --webcam 1']
ros_cmd=['roscore']
...
#txt写入的文件名
filename = '/home/cxking/AlphaPose/examples/res/test/programing.txt'
...
def alphapose1(cmd):
    ...
    time.sleep(10)       #函数推迟调用线程的运行，可通过参数secs指秒数，表示进程挂起的时间。
...
def jsonwork(file_path):
    ...
    return LWrist,LShoulder,RShoulder

#通过计算关节夹角，输出机械臂实际角度
#https://www.jb51.net/article/164697.htm
def angle(LW,LS,RS,arm):  #输入两个向量
    ...
    print('手臂与肩线夹角：',included_angle,file = sys.stderr)
    #映射到机械臂的二维角度中
    if arm > 0:
        included_angle = 90 - included_angle
    else:
        included_angle = 90 + included_angle
    print('机械臂关节二角度： ',included_angle,file = sys.stderr)
...
def main():
    #处理程序输出数据
    try:
        lw,ls,rs = jsonwork(file_path)   #左手腕和左肩坐标
        print('LW = ',lw,'\n','LS = ',ls,'\n','RS = ',rs,'\n',file=sys.stderr)    #打印
    except IOError:
        print ("Error: 没有找到文件或写入文件失败")

    #得到关节状态索引值
    arm_value = arm_index(lw,ls)  #(手腕，肩膀)
    # print('arm_value = ',arm_value,file=sys.stderr)

    #计算机械臂偏移角度
    arm_angle = angle(lw,ls,rs,arm_value)
    slope_index(arm_angle)
    #计算机械臂偏移斜率
    arm_slope = Slope(arm_angle)
