...
def write_json(all_results, outputpath, form=None, for_eval=False):
    ...
    '''
    json_results = []
    json_results_cmu = {}       # 多目标人体关节检测json结果
    for im_res in all_results:
        ...
        for human in im_res['result']:
            ...
            for n in range(kp_scores.shape[0]):
                keypoints.append(float(kp_preds[n, 0])) #第n个关节点的x坐标
                keypoints.append(float(kp_preds[n, 1])) #第n个关节点的y坐标
                #keypoints.append(float(kp_scores[n]))   #第n个关节点的预测得分
            result['keypoints'] = keypoints
            # #result['score'] = float(pro_scores) 
            # if 'box' in human.keys():
            #     result['box'] = human['box']            # 预测框坐标点
