...
def main():
    ...
    #处理程序输出数据
    a,b,c = jsonwork(file_path)   #左手腕和左肩坐标
    print('LW = ',a,'\n','LS = ',b,'\n','RS = ',c,'\n',file=sys.stderr)    #打印

    #得到关节状态索引值
    arm_value = arm_index(a,b)  #(手腕，肩膀)
    # print('arm_value = ',arm_value,file=sys.stderr)
    # print(arm_index(a,b),file=sys.stdout)
    if arm_value > 0:
        arm = 1
        # sys.exit(arm_value)
    elif arm_value < 0:
