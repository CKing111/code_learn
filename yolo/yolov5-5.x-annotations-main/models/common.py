...
class Focus(nn.Module):
    ...
    def forward(self, x):
        ...
        # x(b,c,w,h) -> y(b,4c,w/2,h/2)  有点像做了个下采样，dim为c
        return self.conv(torch.cat([x[..., ::2, ::2], x[..., 1::2, ::2], x[..., ::2, 1::2], x[..., 1::2, 1::2]], 1))    # cat：切片四次后按dim1（c）进行拼接
                                                                # 切片操作：x[c,w,h],[...,::2,::2]:表示步长为2依次遍历wh维度数据
                                                                # 1::2表示从下表1号开始正序步长为2bianli数据
