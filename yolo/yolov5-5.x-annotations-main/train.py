...
def train(hyp, opt, device):
    """
    :params hyp: data/hyps/hyp.scratch.yaml   hyp dictionary定义的相关超参数
    ...
    if isinstance(hyp, str):
        with open(hyp, encoding='utf-8') as f:  # open(路径+文件名, 读写模式, 编码)
            hyp = yaml.safe_load(f)  # load hyps dict  加载超参信息
    # 日志输出超参信息 hyperparameters: ...
    logger.info(colorstr('hyperparameters: ') + ', '.join(f'{k}={v}' for k, v in hyp.items()))

    # Save run settings
    with open(save_dir / 'hyp.yaml', 'w') as f: # 'w':以写的方式打开
    ...
    if pretrained:
        ...
        with torch_distributed_zero_first(RANK):
            ...
            weights = attempt_download(weights)  # dow               nload if not found locally
        # 加载模型及参数
    ...
    if opt.linear_lr:
        # 使用线性学习率
        lf = lambda x: (1 - x / (epochs - 1)) * (1.0 - hyp['lrf']) + hyp['lrf']  # linear   lrf: 0.2               # 最终学习率, 以one_cycle形式或者线性从lr0衰减至lr0 * lrf
    else:
